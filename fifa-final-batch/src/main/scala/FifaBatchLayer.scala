...
object FifaBatchLayer {
  def main(args: Array[String]) {
    ...
    try {
      ...
      val playerData = spark.sql("SELECT *, current_timestamp() as time_stamp FROM playerdatahive")
      val latest_timestamp = playerData
        .orderBy(desc("time_stamp"))
        .limit(1)
        .select("time_stamp")
        .first()
        .getAs[java.sql.Timestamp](0)
      val inputPlayerData = spark.sql(s"SELECT * FROM newplayerdata WHERE time_stamp > '$latest_timestamp'")
      ...
      spark.sql("DELETE FROM newplayerdata WHERE time_stamp < '$latest_timestamp'")
